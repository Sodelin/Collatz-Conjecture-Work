...
The integrated umbrella and its dependencies are the canonical module inventory. The repository does not
...
The umbrella [`lean/CollatzWork.lean`](lean/CollatzWork.lean) imports all integrated proof modules, including the three stopped-route cores and the fixed-algebra YAH proofs. Statement/data modules are separate trusted interfaces. Three original blind derivations under `research/blind-2026-09-05/` compile separately and must not inflate the count of distinct mathematical contributions.
...
### 1. Remaining scalar-arctic YAH formalization

[`YAHFiniteObstruction.lean`](lean/CollatzWork/YAHFiniteObstruction.lean) now checks the exact fixed algebra, canonical legality/embeddability, 13-row adjacent-edge cancellation, and 8-/50-row two-state labeled additive cancellations. The 13-row lower-bound contradiction retains its explicit cofinality premise. The fixed-algebra symbol/edge zero identities support their exact ordered-group conclusions.

This does not formalize the separate 11-/22-rule scalar-arctic full cancellations or the Farkas/RUP top-checker soundness and full-versus-top source semantics. Those remain distinct targets. It also does not establish a 70-scheme classification, arbitrary labeling, matrices, or strategy-dependent reachability.

### 1a. Bounded-alphabet endpoint gate

The [endpoint characterization](proof-search/routes/F_bounded_alphabet_endpoint_residue_gate.md) remains a prose theorem with finite replay. Its converse must explicitly reconstruct integral, positive, odd predecessors and exact valuations. Formalizing the gate will not decide it for a new infinite code.
...
## Additional integrated formal boundaries

| Module | Checked scope | Limit |
|---|---|---|
| [YAHFiniteObstruction](lean/CollatzWork/YAHFiniteObstruction.lean) | Fixed-algebra 13/8/50-row finite additive certificates and stated generic consequences | Not scalar-arctic full/top, arbitrary labels, or global dynamic reachability |
| [BranchingCenter](lean/CollatzWork/Disproof/BranchingCenter.lean) | Eliminated natural center-consistency equation forces equal labels | Rational-center interpretation is a separate prose bridge |
| [FiniteResidueFirstIntegral](lean/CollatzWork/Disproof/FiniteResidueFirstIntegral.lean) | Invariant/commutator/transitivity group-action core | Modulus-factor and affine arithmetic application is prose |
| [PolynomialRatchet](lean/CollatzWork/Disproof/PolynomialRatchet.lean) | Odd normal form, leading telescope, nonresonant arithmetic gain and degree-sum consequences | Polynomial transport/normalization bridge is prose; resonant base3 survives |
| [PrefixCollision](lean/CollatzWork/PrefixCollision.lean) | Finite actual shortcut parity collision and residue separation | Full odd-word/Thue–Morse analytic theorem is not formalized |
| [AffineRepetition](lean/CollatzWork/AffineRepetition.lean) | Fixed affine recurrence denominator budget and conditional numerical bound | No universal repeated-block premise |
| [FinitePaletteObstruction](lean/CollatzWork/FinitePaletteObstruction.lean) | Fixed finite palette of uniformly bounded direct descent promises cannot cover all positive starts | Does not exclude unbounded macros, ranked recursion or general coalescence |

The full module inventory is also preserved in the release source inventory. The [consolidated checkpoint](CONSOLIDATION_2026-09-05.md) maps source contributions to their proof and publication boundaries.
