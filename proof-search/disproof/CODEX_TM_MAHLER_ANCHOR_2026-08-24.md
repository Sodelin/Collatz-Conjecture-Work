...
**Current status:** `EXCLUDED_POSITIVE_REALIZATION` / `HISTORICAL_CONDITIONAL_DERIVATION` / `NO DISPROOF`

> The [2026-09-05 prefix-return exclusion](TM_Prefix_Return_Exclusion_2026-09-05.md) proves that this exact `(1),(2)` Thue–Morse valuation code has no positive realization. The conditional calculations below remain valid historical work; their former positive-membership gate is now closed negatively. The full newer exclusion is analytic, with narrower Lean supporting lemmas, and does not prove Collatz.
...
## 4. Historical positive-membership gate — now excluded
