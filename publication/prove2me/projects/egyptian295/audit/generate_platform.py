...
class Generator:
    ...
    def generate(self):
        ...
        for m in sorted(self.defmods):
            ...
            key='def:'+m
            payload=dict(definition_name=slug(m),definition_title=m+' definitions',natural_language_description='Definitions and necessary proof-bearing construction material from '+m+'.',definition=content,tags=['sodelin-egyptian295'],source=repo+'/blob/'+commit+'/'+self.mods[m]['path'])
        ...
        for k in sorted(self.nodes,key=lambda k:self.node_names[k]):
            ...
            payload=dict(theorem_name=name,theorem_title=(claim['title'] if claim else name),natural_language_statement=(claim['statement']+' Limitations: '+claim['limitations'] if claim else docstr or 'Exact formal statement is authoritative. Metadata review pending.'),preamble=preamble,formal_statement=statement,tags=['sodelin-egyptian295'],source=source)
        ...
        (self.out/'lakefile.toml').write_text('name = "sodelin_egyptian295_staging"\nversion = "0.1.0"\ndefaultTargets = ["Definitions", "Theorems", "Solutions"]\n\n[[lean_lib]]\nname = "Definitions"\nglobs = ["Definitions.**"]\n\n[[lean_lib]]\nname = "Theorems"\nglobs = ["Theorems.**"]\n\n[[lean_lib]]\nname = "Solutions"\nglobs = ["Solutions.**"]\n')
